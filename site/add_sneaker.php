...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->affected_rows > 0) {
        $newSneakerId = $stmt->insert_id; // Получаем ID нового кроссовка
        $response = array('success' => true, 'id' => $newSneakerId);
    } else {
        $response = array('success' => false, 'error' => 'Error adding sneaker: ' . $conn->error);
    }
